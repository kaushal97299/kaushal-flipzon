...
const Home1 = () => {
  ...
  useEffect(() => {
    const fetchCarouselImages = async () => {
      try {
        const response = await axios.get("https://ecommerce-atbk.onrender.com/api/carousel/images");
          ...
          {carouselImages.length === 0 ? (
            ...
            carouselImages.map((src, index) => (
              <div key={index} className={`carousel-item ${index === 0 ? "active" : ""}`}>
                <img src={src} className="d-block w-100 carousel-img" alt={`carousel-${index}`} />
              </div>
            ))
          )}
        </div>
        ...
        {loading ? (
          ...
          <div className="category-grid">
            {categories.map(([category, image]) => (
              ...
              >
                <img
                  src={`https://ecommerce-atbk.onrender.com/uploads/${image}`}
                  alt={category}
