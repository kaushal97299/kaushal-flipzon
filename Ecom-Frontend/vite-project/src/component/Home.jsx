...
const ProductCardList = ({ newProduct }) => {
  ...
  const fetchAllProductRatings = async (products) => {
    ...
    try {
      for (const product of products) {
        const response = await axios.get(`https://ecommerce-atbk.onrender.com/api/reviews/rew/${product._id}`);
  ...
  const fetchProductRating = async (productId) => {
    try {
      const response = await axios.get(`https://ecommerce-atbk.onrender.com/api/reviews/rew/${productId}`);
      const reviews = response.data;
      const total = reviews.reduce((sum, review) => sum + review.rating, 0);
